...
class ViewListBaseActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var times = 0
        val list = listOf(
            ItemListBase("基础操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, times++)) },
            ItemListBase("独立操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, itemModel.item.times++)) },
            ItemListBase("基础操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, times++)) },
            ItemListBase("基础操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, times++)) },
            ItemListBase("基础操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, times++)) },
            ItemListBase("基础操作") { easeHolder, itemModel -> easeHolder.setText(R.id.item_simple_name, String.format("%s 点击了%d", itemModel.item.name, times++)) },
